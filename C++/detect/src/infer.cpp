...
YoloDetector::YoloDetector(const std::string trtFile, int gpuId,
                           float nmsThresh, float confThresh, int numClass)
    : trtFile_(trtFile), nmsThresh_(nmsThresh), confThresh_(confThresh),
      numClass_(numClass) {
  ...
  nvinfer1::Dims outDims =
      context->getTensorShape(engine->getIOTensorName(1)); // [1, 84, 8400]
...
void YoloDetector::get_engine() {
  if (access(trtFile_.c_str(), F_OK) == 0) {
    ...
    if (bINT8Mode) {
      ...
      int batchSize = 8;
      pCalibrator = new Int8EntropyCalibrator2(batchSize, kInputW, kInputH,
                                               calibrationDataPath.c_str(),
                                               cacheFile.c_str());
    ...
    if (!parser->parseFromFile(onnxFile.c_str(),
                               int(gLogger.reportableSeverity))) {
    ...
    profile->setDimensions(inputTensor->getName(), OptProfileSelector::kMIN,
                           nvinfer1::Dims{4, {1, 3, kInputH, kInputW}});
    ...
    IHostMemory *engineString =
        builder->buildSerializedNetwork(*network, *config);
    std::cout << "Succeeded building serialized engine!" << std::endl;

    runtime = createInferRuntime(gLogger);
    engine = runtime->deserializeCudaEngine(engineString->data(),
                                            engineString->size());
    if (engine == nullptr) {
    ...
    engineFile.write(static_cast<char *>(engineString->data()),
                     engineString->size());
...
std::vector<Detection> YoloDetector::inference(cv::Mat &img) {
  ...
  transpose((float *)vBufferD[1], transposeDevice, OUTPUT_CANDIDATES,
            numClass_ + 4, stream);
  // convert [1 8400 84] to [1 7001]
  decode(transposeDevice, decodeDevice, OUTPUT_CANDIDATES, numClass_,
         confThresh_, kMaxNumOutputBbox, kNumBoxElement, stream);
  // cuda nms
  nms(decodeDevice, nmsThresh_, kMaxNumOutputBbox, kNumBoxElement, stream);

  CHECK(
      cudaMemcpyAsync(outputData, decodeDevice,
                      (1 + kMaxNumOutputBbox * kNumBoxElement) * sizeof(float),
...
void YoloDetector::draw_image(cv::Mat &img,
  ...
  for (size_t i = 0; i < inferResult.size(); i++) {
    ...
    std::string labelStr =
        className + " " + std::to_string(inferResult[i].conf).substr(0, 4);

    cv::Size textSize =
        cv::getTextSize(labelStr, cv::FONT_HERSHEY_PLAIN, 1.2, 2, NULL);
    ...
    cv::putText(img, labelStr, cv::Point(r.x, r.y - 2), cv::FONT_HERSHEY_PLAIN,
