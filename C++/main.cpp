...
int run(char *videoPath) {
  ...
  cv::VideoWriter writer("result.mp4", VideoWriter::fourcc('m', 'p', '4', 'v'),
                         fps, Size(img_w, img_h * 2));
  ...
  double yolo_total_time = 0.0, depth_total_time = 0.0;
  while (true) {
    if (!cap.read(img)) {
      break;
    }
    ...
    std::thread depth_thread([&] { result_depth = depth_model.predict(img); });

    // yolo inference

    std::vector<Detection> res;
    std::thread yolo_thread([&] { res = detector.inference(img); });
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      if (isTrackingClass(classId)) {
        cv::Rect_<float> rect(bbox[0], bbox[1], (bbox[2] - bbox[0]),
                              (bbox[3] - bbox[1]));
    ...
    for (int i = 0; i < output_stracks.size(); i++) {
      ...
      if (tlwh[2] * tlwh[3] > 20) {
        ...
        std::string label = cv::format("%s #%d", vClassNames[class_id].c_str(),
    ...
    cv::putText(img,
                cv::format("frame: %d fps: %d num: %ld", num_frames, show_fps,
                           output_stracks.size()),
  ...
  std::cout << "FPS: "
            ...
            << std::endl;
  // 循环结束后输出平均值
  std::cout << "Average YOLO time: " << (yolo_total_time / num_frames) << " ms"
            << std::endl;
  std::cout << "Average Depth time: " << (depth_total_time / num_frames)
            << " ms" << std::endl;
